...
export default async function ExperienceSection({experiences}: {experiences: GetExperiencesType | undefined}) {
    ...
    return (
        <section id="experienceSection" className="w-full h-fit flex flex-col text-slate-200 snap-center">
            <span className="text-4xl text-green-400">Experience &#123;</span>
            <div>
                {experienceData.map(experience => (
                    <div key={experience.id}>
                        ...
                        <p className="text-lg text-white">{experience.employed_since ? new Date(experience.employed_since).toLocaleDateString() : ''}</p>
                        <p className="text-lg text-white">{experience.employed_to ? new Date(experience.employed_to).toLocaleDateString() : ''}</p>
                    </div>
                ))}
            </div>
