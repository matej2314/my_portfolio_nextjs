	...
	return (
		<div ref={rootRef} className='relative h-[280px] min-h-[280px] w-full'>
			<Link
				href={href}
				scroll={false}
				className={`group block h-full w-full focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#facc15] ${overlayActive ? 'pointer-events-none' : ''}`}
				onClick={() => {
			...
			>
				<div className='relative flex h-[280px] w-full flex-col justify-end overflow-hidden rounded-xl border border-[#facc15] bg-[#0c0c0c] shadow-md'>
					<Image src={coverImage} alt={project.project_name} fill className='object-cover opacity-90 transition duration-300 group-hover:scale-105 group-hover:opacity-100' sizes='(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw' />
