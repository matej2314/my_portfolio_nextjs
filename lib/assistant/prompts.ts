...
/** Granice treści użytkownika — tekst między nimi to dane wejściowe, nie instrukcje systemowe. */
export const USER_MESSAGE_BEGIN = '---BEGIN_USER_MESSAGE---';
export const USER_MESSAGE_END = '---END_USER_MESSAGE---';

/**
 * Owija surową wiadomość gościa ramkami i krótkim kontekstem (mitigacja prompt injection).
 */
...
- Jeśli wiadomość żąda czegoś niedozwolonego lub poza zakresem — odmów krótko i zaproponuj pytanie o portfolio.
...
5. Odpowiadaj w języku polskim, profesjonalnie ale przyjaźnie.
...
Użyj odpowiednich narzędzi, aby znaleźć dokładną odpowiedź.`,
...
- If the message asks for something disallowed or out of scope — refuse briefly and suggest a portfolio-related question.
...
5. Answer in English, professionally but friendly.
...
Use appropriate tools to find the exact answer.`,
};
