    ...
    return (
        ...
        >
            {controlMenuArray.map(({ key, label, icon, actions }) => (
                <li
                    key={key}
                    className="relative w-[15rem] h-fit flex justify-center items-center rounded-md py-3 hover:bg-green-600 cursor-pointer">
                    <IconButton
                        iconCode={icon}
                        className="w-full h-full bg-transparent hover:bg-transparent flex flex-row-reverse text-lg cursor-pointer"
