...
export default function SkillSelector({ clickAction, selectedCategory, categories }: SkillSelectorProps) {
	return (
		<motion.ul initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, ease: easeOut }} viewport={{ amount: 0.4, once: true }} className='w-full h-fit grid grid-cols-3 gap-2 min-[450px]:grid-cols-4 text-left xl:pl-[0.6rem]'>
